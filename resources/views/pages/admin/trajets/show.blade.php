                ...
                <div class="card card-cascade narrower">
                    ...
                    <div class="card-body card-body-cascade text-center">
                        ...
                        @foreach ($singleCourse as $item)
                            @if ($item->id != null)
                                <img src="{{ asset('storage/' . $item->id) }}" alt="Photo de {{ $item->id }}"
                                    class="mb-3 mx-auto col-md-12" />
                            @else
                                <img src="{{ asset('bg.jpg') }}" alt="Photo de {{ $item->id }}"
                                    class="mb-3 mx-auto col-md-12" />
                            @endif
                                ...
                                <div class="row flex-center">
                                    <form action="{{ url('user/delete-image') }}" method="POST">
                                        ...
                                        <button type="submit" class="btn btn-danger btn-rounded btn-sm"
                                            onclick="return confirm('Voulez-vous supprimer la photo de l\'utilistateur {{ $item->id }} ?')">
                                            Supprimer
                                        </button>
                                    </form>
                                </div>
            ...
            <div class="col-lg-8 mb-4">
                ...
                <div class="card card-cascade narrower">
                    ...
                    <div class="card-body card-body-cascade text-center">
                        ...
                        <!-- Edit Form -->
                        @foreach ($singleTrajet as $item)
                            <form action="{{ route('trajet.update', $item->id) }}" method="post">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="row">
                                        ...
                                        <div class="md-form mb-0">
                                            <div class="md-form mb-0">
                                                <input type="text" id="ville_depart" name="ville_depart"
                                                    class="form-control @error('ville_depart') validate @enderror"
                                                    value="{{ $item->ville_depart ?? old('ville_depart', '') }}">
                                    ...
                                    <div class="col-md-6">
                                        <div class="md-form mb-0">
                                            <input type="text" id="ville_darrivee" name="ville_darrivee" class="form-control validate"
                                                value="{{ $item->ville_darrivee ?? old('ville_darrivee', '') }}">
                                            <label for="ville_darrivee" data-error="wrong" data-success="right">Ville d'arrivee</label>
                                            @error('ville_darrivee')
                                                <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-md-12 text-center my-4">
                                        <input type="submit" value="Modifier le trajet" class="btn btn-info btn-rounded">
                                    </div>
                                </div>
                                <!-- Fourth row -->
